...
class HermesClient {
  ...
  async runConversation(userMessage: string, conversationHistory?: Message[]): Promise<Message[]> {
    // 模拟 hermes-agent 的对话运行
    const response: Message = {
      role: 'assistant',
      content: `Hermes response to: ${userMessage}`,
      finishReason: 'stop'
    };
...
export class HermesAdapter extends BaseAdapter {
  ...
  async cleanup(): Promise<void> {
    // 清理 hermes-agent 相关资源
    console.log('Cleaning up Hermes resources');
  }
}

export class HermesAdapterFactory {
  /**
   * 创建 agent 适配器实例
   */
  async createAdapter(config: AgentConfig): Promise<HermesAdapter> {
