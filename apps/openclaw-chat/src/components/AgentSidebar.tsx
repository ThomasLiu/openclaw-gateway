...
function pickLatestSessionRowForAgent(
  ...
  return agentSessions.reduce((latest, s) => (s.updatedAt > latest.updatedAt ? s : latest));
}
...
export function AgentSidebar({ agentId, onSelectAgent, gatewaySessions }: AgentSidebarProps) {
  ...
  return (
    <aside className="w-48 flex-shrink-0 bg-zinc-900 border-r border-zinc-800 flex flex-col overflow-hidden">
      <div className="p-2 text-xs font-semibold text-zinc-400 uppercase tracking-wider">Agent</div>
      {loading ? (
        <div className="px-3 py-2 text-zinc-500 text-sm">加载中…</div>
      ) : (
        <nav className="flex-1 overflow-y-auto">
          {agents.map((agent) => {
            const previewSession = pickLatestSessionRowForAgent(gatewaySessions, agent.id);
